...
const collectRecipients = async () => {
    ...
    // Acciones estrategicas: solo se notifica si tienen responsable(s)
    // configurado(s); si una accion no tiene ninguno, simplemente se omite.
    const acciones = await Accion.find({}, 'codigo nombre responsable responsable_email responsables proyecto_id')
        .populate({
            path: 'proyecto_id',
        ...
        .lean();
    const accionesVigentes = acciones.filter((accion) => accion.proyecto_id);

    for (const accion of accionesVigentes) {
        if (Array.isArray(accion.responsables) && accion.responsables.length > 0) {
            accion.responsables.forEach((r) => personNames.push(r.nombre));
        } else {
            personNames.push(accion.responsable);
        }
    }
    ...
    for (const accion of accionesVigentes) {
        ...
        const item = {
            tipo: 'Acción estratégica',
            codigo: accion.codigo,
            nombre: accion.nombre,
            macro: macro ? `${macro.codigo} - ${macro.nombre}` : '',
            proyecto: proyecto ? `${proyecto.codigo} - ${proyecto.nombre}` : '',
        };
        if (Array.isArray(accion.responsables) && accion.responsables.length > 0) {
            for (const responsable of accion.responsables) {
                const recipient = resolveUserRecipient(usersByName, responsable.nombre, responsable.email);
                addRecipient(recipients, { email: recipient.email, name: recipient.name, role: 'Responsable de acción estratégica', item });
            }
        } else {
            const recipient = resolveUserRecipient(usersByName, accion.responsable, accion.responsable_email);
            addRecipient(recipients, { email: recipient.email, name: recipient.name, role: 'Responsable de acción estratégica', item });
        }
    }
    ...
    const rows = visibleItems.map((item) => `
        <tr>
            <td style="padding:10px 12px;border-bottom:1px solid #e2e8f0;">
                ...
                ${item.proyecto ? `<div style="font-size:12px;color:#64748b;margin-top:3px;">Proyecto: ${escapeHtml(item.proyecto)}</div>` : ''}
                ${item.macro ? `<div style="font-size:12px;color:#64748b;margin-top:2px;">Macroproyecto: ${escapeHtml(item.macro)}</div>` : ''}
            </td>
